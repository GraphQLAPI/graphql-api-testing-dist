...
class CPTBlockAttributesAdminRESTController extends AbstractAdminRESTController
{
     ...
     * @param string $customPostID
     */
    protected function validateCustomPost($customPostID)
    {
     ...
     */
    protected function getCustomPost($customPostID)
    {
        if ($customPostID <= 0) {
            return new WP_Error(
     ...
     * @param int $customPostID
     */
    protected function prepareItemsForResponse($customPostID, $blocks): array
    {
        $items = [];
     ...
     * @param int $customPostID
     */
    protected function prepareItemForResponse($customPostID, $block)
    {
     ...
     * @return array<string,mixed>
     */
    protected function prepareItem($block): array
    {
        return [
     ...
     */
    public function getNonExistingBlockError($customPostID, $blockNamespace, $blockID): WP_Error
    {
        $errorData = [
            Params::STATE => [
                Params::CUSTOM_POST_ID => $customPostID,
     ...
     * @param string $blockID
     */
    protected function getBlock($blockNamespace, $blockID, $blocks): ?array
    {
        [$blockNamespacedName, $blockPosition] = $this->getBlockNamespacedNameAndPosition($blockNamespace, $blockID);
        $blockCounter = 0;
        foreach ($blocks as $block) {
     ...
     * @param string $blockID
     */
    protected function getBlockNamespacedNameAndPosition($blockNamespace, $blockID): array
    {
     ...
     * @param int $customPostID
     */
    protected function prepareLinks($customPostID, $block): array
    {
        /** @var string */
    ...
    /**
     * @param string $blockNamespacedName
     * @param int $blockPosition
     */
    protected function getBlockID($blockNamespacedName, $blockPosition): string
    {
        if ($blockPosition === 0) {
    ...
    public function updateItem($request)
    {
        ...
        try {
            ...
            if ($blockAttributeValues === null) {
                return new WP_Error(
                    '1',
                    sprintf(__('Property \'%s\' is not JSON-encoded properly', 'graphql-api-testing'), Params::JSON_ENCODED_BLOCK_ATTRIBUTE_VALUES),
                    [
                        Params::STATE => [
                            Params::CUSTOM_POST_ID => $customPostID,
                            Params::BLOCK_NAMESPACE => $blockNamespace,
            ...
            $response->message = $blockPosition === 0
                ? sprintf(__('Attributes for block \'%s\' have been updated successfully', 'graphql-api-testing'), $blockNamespacedName)
                : sprintf(
